...
「たなばんちゃん」は、電子工作やハードウェア開発で使用する電子部品（IC、抵抗、コンデンサなど）を効率的に管理するためのWebアプリケーションです。Google スプレッドシートをデータベースとして利用し、OCR（光学文字認識）を用いた自動入力機能や、高度な検索・編集機能を提供します。
...
* **秋月電子からの自動取得:**
  * 「販売コード（例：`P-14526`）」を入力、またはOCRで読み取ることで、秋月電子のWebサイトから部品の名称、データシートのURL、商品リンクなどの詳細データを自動で取得・入力します。
* **入力補助（オートコンプリート）:**
  * カテゴリやメーカー名、保管場所などは、過去に登録した既存のデータをもとに入力候補をサジェスト（自動補完）します。

### 2. 重複チェックと柔軟なデータ更新
* **既存データの読み込み・上書き:**
  * 同じ「型番」や「販売コード」を入力すると、すでにデータベースに登録されている情報を自動的に検出して確認メッセージを表示します。既存データを読み込んで、必要な項目だけを変更して上書き更新することができます。
* **数量の四則演算入力（計算機能）:**
...
2. **アプリでの設定:**
   * 「たなばんちゃん」のWebアプリ右上の「歯車アイコン（設定）」を開きます。
